...
def calculate_select_in(productions, firsts, follows):
    ...
    for non_terminal in productions:
        for production in productions[non_terminal]:
            select_key = f"{non_terminal} -> {production}"  # SELECT(S->AB)
            ...
            if 'ε' in first_set:
                first_set.remove('ε')
                first_set.update(follows[non_terminal])  # 更新，添加FOLLOW集合中的元素
